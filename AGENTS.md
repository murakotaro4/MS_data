...
- キャッシュ: `ms_data/net/cache_http.py`（TTL・If-None-Match/If-Modified-Since 対応）。保存先 `cache/html/<slug>.html` + `*.meta.json`。
- レート制限: 既定 2.0 req/sec。atwiki への負荷を考慮し過度な緩和は避ける。2回目以降は `NO_NET=1` でキャッシュのみ利用可。
